        ...
        <Menu as="div" className={props.mobile ? 'relative' : 'mx-3 relative'}>
          ...
          <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
            <Menu.Items className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white focus:outline-none">
              ...
              <Menu.Item>
                {({ active }) => <Link href="/my-finances" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">My Finances</Link>}
              </Menu.Item>
              <Menu.Item>
                {({ active }) => <Link href="/jumpseat" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Jumpseat</Link>}
              </Menu.Item>
              <Menu.Item>
                {({ active }) => <a target="_blank" href="/https://productific.com/@BushDivers#voting" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Vote on Features</a>}
              </Menu.Item>
              <Menu.Item>
